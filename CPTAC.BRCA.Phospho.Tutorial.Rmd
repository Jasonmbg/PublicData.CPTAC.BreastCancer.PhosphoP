...
dat.phospho.mod <- dat.phospho %>% dplyr::select(-c(NAME, DESCRIPTION, GENE_SYMBOL, PHOSPHOSITES)) %>% mutate(Feature_mod = str_extract(ENTITY_STABLE_ID, "[^_]*_[^_]*")) %>% mutate(Feature_final=str_sub(Feature_mod,1,nchar(Feature_mod)-1)) %>%
dplyr::distinct(Feature_final,.keep_all=TRUE) %>%
column_to_rownames("Feature_final") %>% select(-c(Feature_mod,ENTITY_STABLE_ID)) %>% 
...
comp1_stat_cptac_ptm <- limma::topTable(fit3, coef=1, number=nrow(fit3), adjust.method="fdr", sort.by="none") %>% dplyr::select(t) %>% rename(ER_Pos_vs_ER_Neg_t_stat=t) %>% drop_na() %>% 
rownames_to_column() %>% separate("rowname", sep="_", into=c("GeneSymbol","PhosID"))
